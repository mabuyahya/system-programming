...
- the browser also has a javascript interpreter, so when you run a javascript code in the browser, the browser's javascript interpreter reads the javascript code line by line, tokenizes each line, and executes it directly by calling the pre-written functions that handle different operations(these functions are written in c++ and compiled to machine code).
- DBMS consider a query interpreter, the query interpreter is responsible for parsing and executing database queries. It reads the query text, tokenizes it, and then translates it into a series of operations that can be performed on the database. These operations are often implemented as pre-written functions in a lower-level language like C, which are then called by the interpreter to interact with the database.